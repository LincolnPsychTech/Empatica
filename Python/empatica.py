...
def connect(port=28000):
    serverAddress = ("127.0.0.1", port) # Setup server ip and port, by defaul it uses localhost
...
def subscribe(sock, *sensors):
    ...
    streamsDictionary = {   # Dictionary of possible sensors and corresponding commands
...
def getval(sock):
    ...
    for row in spltresp:
        ...
        try:
            val.append({
                    "Time": float(spltrow[1]),
                    spltrow[0].strip("E4_"): [float(v) for v in spltrow[2:]] 
                    })
        except:
            val.append({
                    "Time": [],
...
def disconnect(sock):
    sock.sendall("device_disconnect".encode() )
    resp = sock.recv(1024)
    sock.settimeout(10)
